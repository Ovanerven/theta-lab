...
from src.models.simple_ode_rnn_7_9 import SimpleRNN7
from src.models.simple_ode_rnn_7_9 import SimpleRNN9
...
def loss_fn_per_species(pred: torch.Tensor, y_seq: torch.Tensor) -> torch.Tensor:
    """Per-species MSE in log1p space. Returns (P,) tensor."""
    log_y = torch.log1p(y_seq)
    log_pred = torch.log1p(pred)
    # mean over batch and time, keep species dim -> (P,)
...
def Training_loop(
    ...
    *,
    ...
    # Model instantiation (unified interface for all model variants)
    MODEL_TYPE_MAP = {SimpleRNN: "5state", SimpleRNN7: "7state", SimpleRNN9: "9state"}
    P_FULL_MAP     = {SimpleRNN: P,       SimpleRNN7: 7,        SimpleRNN9: 9       }

    model_type = MODEL_TYPE_MAP[model_cls]
    P_full     = P_FULL_MAP[model_cls]

    # obs_indices: positions in the reduced state — always [0 .. P-1]
    # (dataset already selects only observed species, so y0 is dense)
    obs_indices_tensor = torch.arange(P, dtype=torch.long)

    # jump matrix: u_k ∈ R^U → Δx ∈ R^{P_full}  (identity when control == obs)
    jump = torch.zeros(U, P_full, dtype=torch.float32)
    for u in range(min(U, P_full)):
        jump[u, u] = 1.0

    model = model_cls(
        U,
        obs_indices=obs_indices_tensor,
        P_full=P_full,
        hidden=cfg.hidden,
    ...
    do_script = False  # Temporarily disabled to avoid TorchScript cache issues
    if do_script:
        try:
    ...
    train_losses = []
    val_losses = []
    val_species_losses: list = []  # list of (P,) arrays, one per epoch
    ...
    for ep in range(1, cfg.epochs + 1):
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for y0, u_seq, dt_seq, y_seq in val_loader:
                    ...
                    # pred, _, _ = model(y0, u_seq, dt_seq, y_seq=None, teacher_forcing=False)
                    ...
                    sp = loss_fn_per_species(pred, y_seq).cpu()  # (P,)
                    species_total = sp if species_total is None else species_total + sp
                    v_batches += 1

            val_loss = val_total / max(1, v_batches)
            val_losses.append(val_loss)

            if species_total is not None:
                val_species_losses.append((species_total / max(1, v_batches)).numpy())
        ...
        if val_loss is None:
            ...
            print(f"ep {ep:4d} | train {train_loss:.6f} | val {val_loss:.6f} | best {best_val:.6f}{sp_str} | {epoch_time:.2f}s")
        ...
        if cfg.save_checkpoint_every > 0 and ep % cfg.save_checkpoint_every == 0:
            ...
            torch.save({"state_dict": state, "epoch": ep, "cfg": cfg.__dict__, "model_type": model_type}, ckpt_path)
    ...
    np.savez(
        ...
        val_species_losses=np.array(val_species_losses) if val_species_losses else None,
    ...
    if cfg.save_path is not None:
        ...
        # Remove torch.compile prefix if present
        state = {k.replace("_orig_mod.", ""): v for k, v in state.items()}
        torch.save({"state_dict": state, "cfg": cfg.__dict__, "best_val": best_val, "model_type": model_type}, cfg_path)
        print(f"Saved best model to {cfg_path}")
    
    # save config.yaml (especially useful for experiments)
    if cfg.exp_name is not None:
        exp_dir = Path(cfg.log_dir).parent  # Get actual exp dir (with timestamp) from log_dir
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="5state", choices=["5state", "7state", "9state"], help="Model architecture to use")
    args = parser.parse_args()

    # Select model class based on --model argument
    model_map = {
        "5state": SimpleRNN,
        "7state": SimpleRNN7,
        "9state": SimpleRNN9,
    ...
    if args.exp_name is not None:
        # use experiments/{timestamp}_{exp_name}/ structure
        exp_dir = Path("experiments") / f"{timestamp}_{args.exp_name}"
        exp_dir.mkdir(parents=True, exist_ok=True)
        
        log_dir = str(exp_dir / "logs")
        save_path = str(exp_dir / "model.pt")
        print(f"Using experiment structure: experiments/{timestamp}_{args.exp_name}/")
    else:
    ...
    Training_loop(args.data, cfg=cfg, model_cls=model_cls)
    ...
    if not args.no_plot:
        ...
        try:
            ...
            if args.exp_name is not None:
                plots_dir = str(Path(save_path).parent / "plots")
            else:
