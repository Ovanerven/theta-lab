...
def loss_fn(
    ...
    Composable extensions used by the weighted composite loss:
    ...
    if use_log_loss:
        if clamp_min > 0.0:
            # Floor both pred and y at clamp_min before log1p,
...
def endpoint_mse(
    ...
    `clamp_min`: floor pred/y before log1p (0 disables).
...
class TrainConfig:
    ...
    lift_skip: bool = False               # drop the lift MLP and feed feat→GRU directly
    gru_variant: str = "nn_gru"           # "nn_gru"
    gru_init: str = "default"             # "default" | "orthogonal" (orthogonal_ + xavier_ + zeros)
    ...
    theta_head_tau: float = 1.0           # log_gamma sigmoid temperature
    ...
    # "Gate the observation": per-sample, per-channel min-subtraction on
    ...
    # "Gate the parameter" (ablation knob): pin chosen entries of θ(t) to
    ...
    # --- composable loss knobs (default None → unchanged) ---
    ...
    # channel's weight by factor. Example: pm second-half ×3:
    ...
    # raw state indices. Example: pm endpoint with weight 0.1:
    #   lambda_endpoint: 0.1
    #   endpoint_channels: [5]
    ...
    # Optionally divide the final composite by the number of nonzero
    # loss-weight channels (e.g. =3: mm + pm-body + pm-endpoint).
    # Set this to that count to apply the /N normaliser; leave None
    ...
    # Typical value 1.0 (floors small values at 1.0 before log1p). 0.0
...
def train(cfg: TrainConfig, *, no_plot: bool = False, plot_samples: int = 5, plot_sample_idx: int = 0) -> None:
    ...
    # The K-anchor sparse-θ wrapper accepts two extra kwargs. Only pass them to
    # that model so the other model classes' signatures stay untouched.
    SPARSE_THETA_MODELS = {"ode_rnn_sparse_theta"}
    sparse_theta_kwargs = {}
