...
class MOF_ZIF8_ThreeStep(MechanisticScaffold):
    """
    ...
    """
    def __init__(self):
        super().__init__(P=7, theta_dim=19)
        self.state_names = ["M","L","P","A","N","R","T"] #NO S
    
    def _hill(self, x: torch.Tensor, n: torch.Tensor, K: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
            x = torch.clamp_min(x, 0.0)
            n = torch.clamp_min(n, 1e-6)
            K = torch.clamp_min(K, eps)
            xn = x.pow(n)
            Kn = K.pow(n)
            return xn / (Kn + xn + eps)

    def _safe_log(self, x: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
        return torch.log(torch.clamp_min(x, eps))

    def _cnt_like_nucleation(
        self,
        S: torch.Tensor,
        kJ: torch.Tensor,
        Bcnt: torch.Tensor,
        ...
        S_pos = torch.clamp_min(S, 1.0 + eps)
        lnS = self._safe_log(S_pos, eps=eps)
        base = torch.clamp_min(S - 1.0, 0.0)
        return kJ * torch.exp(-Bcnt / (lnS * lnS + eps)) * base.pow(p)

    def _surface_area_spheres(
        self,
        N: torch.Tensor,
        R: torch.Tensor,
        shape_factor: torch.Tensor,
    ) -> torch.Tensor:
        N = torch.clamp_min(N, 0.0)
        R = torch.clamp_min(R, 0.0)
        shape_factor = torch.clamp_min(shape_factor, 0.0)
        return shape_factor * (4.0 * math.pi) * R.pow(2) * N

    def forward(self, y: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
        M, L, P, A, N, R, T = y.unbind(dim=-1)
        (
            kML, aML, bML, kLP, kPA, kAP, kAN, nA, KA,
            kJ, Bcnt, kg, gG, kdiss, chi, shape, kagg, Peq0, Eeq
        ) = theta.unbind(dim=-1)

        M_pos = torch.clamp_min(M, 0.0)
        L_pos = torch.clamp_min(L, 0.0)
        P_pos = torch.clamp_min(P, 0.0)
        A_pos = torch.clamp_min(A, 0.0)
        N_pos = torch.clamp_min(N, 0.0)

        kML = torch.clamp_min(kML, 0.0)
        aML = torch.clamp_min(aML, 0.0)
        bML = torch.clamp_min(bML, 0.0)
        kLP = torch.clamp_min(kLP, 0.0)
        kPA = torch.clamp_min(kPA, 0.0)
        kAP = torch.clamp_min(kAP, 0.0)
        kAN = torch.clamp_min(kAN, 0.0)
        nA = torch.clamp_min(nA, 1e-6)
        KA = torch.clamp_min(KA, 1e-12)
        kJ = torch.clamp_min(kJ, 0.0)
        Bcnt = torch.clamp_min(Bcnt, 0.0)
        kg = torch.clamp_min(kg, 0.0)
        gG = torch.clamp_min(gG, 0.0)
        kdiss = torch.clamp_min(kdiss, 0.0)
        chi = torch.clamp_min(chi, 0.0)
        shape = torch.clamp_min(shape, 0.0)
        kagg = torch.clamp_min(kagg, 0.0)
        Peq0 = torch.clamp_min(Peq0, 1e-12)
        T_eff = torch.clamp(T, 280, 500)

        r_ML = kML * M_pos.pow(aML) * L_pos.pow(bML)
        r_LP = kLP * P_pos

        r_PA = kPA * P_pos
        r_AP = kAP * A_pos
        r_AN = kAN * self._hill(A_pos, n=nA, K=KA)

        Peq = Peq0 * torch.exp(-Eeq / (T_eff + 1e-12))
        S = P_pos / torch.clamp_min(Peq, 1e-12)

        SA = self._surface_area_spheres(N_pos, R, shape_factor=shape)
        J = self._cnt_like_nucleation(S, kJ=kJ, Bcnt=Bcnt, p=0.0)

        G_pos = kg * torch.clamp_min(S - 1.0, 0.0).pow(gG)
        G_neg = kdiss * torch.clamp_min(1.0 - S, 0.0)
        dR_dt = G_pos - G_neg

        consume = chi * SA * G_pos
        returnP = chi * SA * G_neg

        dM = -r_ML + r_LP
        dL = -r_ML + r_LP
        dP = r_ML - r_LP - r_PA + r_AP - consume + returnP
        dA = r_PA - r_AP - r_AN
        dN = J + 0.1 * r_AN - kagg * N_pos.pow(2)
        dT = torch.zeros_like(dM)

        return torch.stack((dM, dL, dP, dA, dN, dR_dt, dT), dim=-1)
