...
def build_pm_time_weights(
    ...
    mode='simple' : ones, except w[-1] = endpoint_weight. Just nudges the final
                    pm residual harder.
    mode='bob'    : mirrors bob_model / train.py:bob_loss for pm —
                      • second half of trajectory (k >= K//2) weighted by tail_weight
                      • last step gets an additional endpoint_weight on top.
                    Defaults (tail=3, endpoint=0.1) match Bob's loss_weight scheme.
    """
...
def main():
    ...
    pf.add_argument("--pm-endpoint-mode", choices=["none", "simple", "bob"],
                    default="none",
                    help="Add extra weight on pm in the loss. "
                         "'simple' weights only pm at the last step by "
                         "--pm-endpoint-weight. 'bob' mirrors bob_model/train.py: "
                         "second half of trajectory gets --pm-tail-weight on pm, "
                         "plus an extra --pm-endpoint-weight at the final step.")
    pf.add_argument("--pm-endpoint-weight", type=float, default=5.0,
                    help="Endpoint pm weight. Default 5.0 for 'simple'; "
                         "match Bob (0.1) for 'bob'.")
    pf.add_argument("--pm-tail-weight", type=float, default=3.0,
                    help="Second-half pm weight in 'bob' mode (default 3.0, "
                         "matches bob_model loss).")
