...
def _fit_greedy(sc, y0_f, y_seq_f, u_seq, dt, jump, lo_t, hi_t,
    ...
    for k in range(K):
        ...
        target = y_seq_f[:, k].index_select(1, obs_idx_t)
...
def _fit_joint(sc, y0_f, y_seq_f, u_seq, dt, jump, lo_t, hi_t,
    ...
    for s in range(0, N, chunk):
        ...
        tgt = yseq_c.index_select(2, obs_idx_t)            # (B,K,n_obs)
...
def main():
    ...
    for sn in scaffolds:
        if sn not in SCAFFOLDS:
            ...
            continue
        sc = SCAFFOLDS[sn]
        print(f">> {LADDER_LABEL.get(sn, sn)}  {sn}  (P={sc.P}, theta_dim={sc.theta_dim})")
    ...
    # summary table + csv
    print("=" * 78)
    print(f"{'Model':<6}{'P':>3}{'theta':>6}{'trajR2_mRNA':>13}{'trajR2_prot':>13}"
          f"{'epR2_mRNA':>11}{'epR2_prot':>11}")
    print("-" * 78)
    csv_path = out_dir / "oracle_fit_summary.csv"
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["scaffold", "label", "P", "theta_dim", "N",
                    "trajR2_mRNA", "trajR2_protein", "nrmse_mRNA", "nrmse_protein",
                    "endpointR2_mRNA", "endpointR2_protein"])
        for r in results:
            tr, ep = r["traj"], r["endpoint"]
            print(f"{r['label']:<6}{r['P']:>3}{r['theta_dim']:>6}"
                  f"{tr['mRNA'][0]:>+13.3f}{tr['protein'][0]:>+13.3f}"
                  f"{ep['mRNA']:>+11.3f}{ep['protein']:>+11.3f}")
            w.writerow([r["scaffold"], r["label"], r["P"], r["theta_dim"], r["N"],
    ...
    # save per-scaffold predictions for plotting
    np.savez_compressed(
        out_dir / "oracle_fit_predictions.npz",
        **{f"{r['label']}_pred": r["pred"] for r in results},
        **{f"{r['label']}_true": r["true"] for r in results},
        **{f"{r['label']}_valid": r["valid"] for r in results},
    )
    print(f"Saved predictions -> {out_dir / 'oracle_fit_predictions.npz'}")
