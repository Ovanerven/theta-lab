...
class OdeRNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        theta_head_transform: str = "log_gamma",      # "log_gamma" (default) | "gamma" (linear-sigmoid)
        theta_head_tau: float = 1.0,                  # temperature for log_gamma sigmoid; 2.3 gives a gentler slope
        head_bottle: bool = False,                    # if True, insert hidden→…→SiLU stack before head (see head_bottle_dims)
        head_bottle_dims: Optional[list] = None,      # bottle layer widths; default [120, 40]
        lift_skip: bool = False,                      # if True, drop the lift MLP and feed feat→GRU directly
        gru_variant: str = "nn_gru",                  # only "nn_gru" is supported
        gru_init: str = "default",                    # "default" (PyTorch defaults) | "orthogonal" (orthogonal_ W_hh + xavier_ W_ih + zeros)
        head_init: str = "default",                   # "default" (PyTorch defaults; respect head_bias_init/head_weight_gain) | "orthogonal" (xavier_ + zeros, unconditional)
        ...
        **kwargs,
        ...
        if gru_variant != "nn_gru":
            raise ValueError(f"gru_variant must be 'nn_gru', got {gru_variant}")
        if gru_init not in ("default", "orthogonal", "supervisor"):
            raise ValueError(f"gru_init must be 'default' or 'orthogonal', got {gru_init}")
        ...
        # Head init: "orthogonal" applies xavier_uniform + zeros UNCONDITIONALLY.
        ...
        if self.head_init in ("orthogonal", "supervisor"):
        ...
        # GRU init: "orthogonal" forces orthogonal_(W_hh) + xavier_uniform_(W_ih) + zeros(biases).
        # nn.GRU stores params under names like "weight_ih_l{k}", "weight_hh_l{k}", etc.
        # "supervisor" is a backward-compatible alias for "orthogonal".
        if self.gru_init in ("orthogonal", "supervisor"):
    ...
    def forward(
        ...
        for k in range(K):
            ...
            # Alternative policy: TF fires at k=0 too (with k-1=-1 wrapping to the
            ...
            y_out[:, k, :] = y
            # When the scaffold defines a wider loss-facing theta layout (e.g. an
