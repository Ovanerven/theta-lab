"""K-anchor sparse-θ wrappers for OdeRNN, OdesLSTM, and OdeRNNBasalV2.
...
Three wrappers are registered:
  - ode_rnn_sparse_theta            (recommended; on top of the regular OdeRNN)
  - ode_slstm_sparse_theta          (on top of OdesLSTM)
  - ode_rnn_basal_v2_sparse_theta   (on top of OdeRNNBasalV2; basal is forced
                                     off in the second integration pass)

All three accept the same two extra kwargs:
...
class OdesLSTMSparseTheta(_SparseThetaMixin, OdesLSTM):
    """K-anchor sparse-θ on top of OdesLSTM."""

    def __init__(self, *, n_theta_anchors: int = 6, anchor_interp: str = "piecewise", **kwargs):
        OdesLSTM.__init__(self, **kwargs)
        self._sparse_init(n_theta_anchors, anchor_interp, use_basal_rk4=False)
    ...
    def forward(
        ...
        return self._anchored_forward(
            OdesLSTM.forward.__get__(self, OdesLSTM),
            y0, u_seq, dt_seq, obs_idx, y_seq,
            teacher_forcing=teacher_forcing, tf_every=tf_every,
            u_transform=u_transform, y_transform=y_transform,
        )
...
class OdeRNNBasalV2SparseTheta(_SparseThetaMixin, OdeRNNBasalV2):
    """K-anchor sparse-θ on top of OdeRNNBasalV2.

    Basal is disabled in the re-integration pass (β = 0). Use this only when you
    want the sparse-θ readout layered on the basal-v2 architecture; for the
    main TXTL ladder runs prefer OdeRNNSparseTheta (`ode_rnn_sparse_theta`).
    """

    def __init__(self, *, n_theta_anchors: int = 6, anchor_interp: str = "piecewise", **kwargs):
        OdeRNNBasalV2.__init__(self, **kwargs)
        self._sparse_init(n_theta_anchors, anchor_interp, use_basal_rk4=True)
    ...
    def forward(
        ...
        return self._anchored_forward(
            OdeRNNBasalV2.forward.__get__(self, OdeRNNBasalV2),
            y0, u_seq, dt_seq, obs_idx, y_seq,
            teacher_forcing=teacher_forcing, tf_every=tf_every,
            u_transform=u_transform, y_transform=y_transform,
            basal_scale=0.0,  # zero out basal during the dense-θ extraction pass
        )
