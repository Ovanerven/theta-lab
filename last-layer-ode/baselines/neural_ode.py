...
class ODERNN(nn.Module):
    """
    Baseline Neural ODE (no mechanistic scaffold).
    A plain MLP parameterises the full vector field directly.
    ...
    def __init__(
        ...
        **kwargs,                    # absorb rhs= and any other scaffold kwargs
    ):
        super().__init__()
    ...
    def _rk4_substeps(
        ...
        y: torch.Tensor,    # (B,P)
        dt: torch.Tensor,   # (B,)
        u_k: torch.Tensor,  # (B,U)
    ) -> torch.Tensor:
        n_sub = self.n_substeps
        hdt = dt.unsqueeze(1) / float(n_sub)   # (B,1)
    ...
    def forward(
        ...
        obs_idx: torch.Tensor,                          # (num_obs,) — empty = no partial TF
